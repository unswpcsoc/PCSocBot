...
var (
	ErrNoTags     = errors.New("no tags found in database")
...
type user struct {
	ID        string
	Tag       string
	Platforms map[string]platform // indexed by name
}

type platform struct {
	Name string
	//TODO: Role string
	Users map[string]user // indexed by user id's
}
...
func (t *TagsAdd) MsgHandle(ses *discordgo.Session, msg *discordgo.Message, args []string) (*CommandSend, error) {
	...
	return NewSimpleSend(msg.ChannelID, "Added tag "+utils.Code(t.Tag)+" for platform "+utils.Code(t.Platform)), nil
}
...
func (t *TagsList) MsgHandle(ses *discordgo.Session, msg *discordgo.Message, args []string) (*CommandSend, error) {
	...
	for _, user := range plt.Users {
		var usr *discordgo.User
		usr, err = ses.User(user.ID)
		if err != nil {
			// silently fail
			continue
		}
...
func (t *TagsPing) Desc() string { return "Ping everyone with the given ping" }
